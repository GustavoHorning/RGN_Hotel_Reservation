...
function checkInputDate() {
    ...
    if (checkinValue === "" && checkoutValue === "") {
        ...
        const checkinDate = new Date(checkinValue.replace(/-/g, '/'));
        const checkoutDate = new Date(checkoutValue.replace(/-/g, '/'));
        const currentDate = new Date();
        currentDate.setHours(0, 0, 0, 0);

        if (checkinDate < currentDate) {
            errorInput(checkin, "A data de check-in deve ser igual ou posterior à data atual.");
            clearError(checkout);
